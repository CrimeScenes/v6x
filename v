...
local predictionMultiplier = config.Preds.PredictionMultiplier
...
local function predictHitPoint(targetPlayer, deltaTime)
    local humanoid = targetPlayer.Character and targetPlayer.Character:FindFirstChild("Humanoid")
    if humanoid then
        local rootPart = humanoid.RootPart or targetPlayer.Character.HumanoidRootPart
        local velocity = rootPart.AssemblyLinearVelocity
        local distance = (rootPart.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude

        -- Adjust prediction factor based on distance and multiplier
        if distance < 50 then
        ...
        predictionFactor = predictionFactor * predictionMultiplier

        -- Offset adjustments for hit-point prediction
        local offset = Vector3.new(0, 0, 0)
        if distance < 50 then
            offset = Vector3.new(0, 1.5, 0) -- Close range
        elseif distance < 200 then
            offset = Vector3.new(0, 1, 0) -- Mid-range
        else
            offset = Vector3.new(0, 0.5, 0) -- Far range
        end

        -- Predict and smooth positions
        local predictedPosition = rootPart.Position + velocity * deltaTime * predictionFactor + offset
        table.insert(lastPositions, 1, predictedPosition)
        ...
        for _, pos in ipairs(lastPositions) do
            avgPosition = avgPosition + pos
        end

        return avgPosition / #lastPositions
    end
...
local function TriggerBotAction()
    if TargetPlayer and TargetPlayer.Character then
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            local predictedHitPoint = predictHitPoint(TargetPlayer, 0.1)
            local screenPos, onScreen = Camera:WorldToViewportPoint(predictedHitPoint)
            if onScreen and isWithinViewEra(predictedHitPoint) then
